...
class pxssh (spawn):
    ...
    def __init__ (self, timeout=30, maxread=2000, searchwindowsize=None, logfile=None, cwd=None, env=None):
        ...
        self.UNIQ_STR = str(hashlib.md5(str(time.time())).hexdigest()).upper()
        self.UNIQUE_PROMPT = "\[PEXPECT_%s\][\$\#] " % self.UNIQ_STR
        self.PROMPT = self.UNIQUE_PROMPT

        # used to set shell command-line prompt to UNIQUE_PROMPT.
        self.PROMPT_SET_SH = "PS1='[PEXPECT_%s]\$ '" % self.UNIQ_STR
        self.PROMPT_SET_CSH = "set prompt='[PEXPECT_%s]\$ '" % self.UNIQ_STR
    ...
    def clear_cache(self, timeout=0.5):
        try:
            self.read_nonblocking(size=10000,timeout=timeout) # GAS: Clear out the cache before getting the prompt
        except TIMEOUT:
            pass
    ...
    def synch_original_prompt (self):
        ...
        self.sendline()
        time.sleep(sleep_time)
        x = self.read_nonblocking(size=1000,timeout=timeout)
        time.sleep(sleep_time)
        self.sendline()
        time.sleep(sleep_time)
        a = self.read_nonblocking(size=1000,timeout=timeout)
        time.sleep(sleep_time)
        self.sendline()
        time.sleep(sleep_time)
        b = self.read_nonblocking(size=1000,timeout=timeout)
    ...
    def login (self,server,username,password='',terminal_type='ansi',original_prompt=r"[#$]",login_timeout=3,port=None,auto_prompt_reset=True):
        ...
        if i==0: 
            ...
            self.sendline("yes")
            i = self.expect(["(?i)are you sure you want to continue connecting", original_prompt, "(?i)(?:password)|(?:passphrase for key)", "(?i)permission denied", "(?i)terminal type", TIMEOUT], timeout=login_timeout)
